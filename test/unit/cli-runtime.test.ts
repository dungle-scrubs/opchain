...
describe("runCli", () => {
  ...
  test("parse errors short-circuit before command dispatch", async () => {
    let dispatchCalls = 0;
    const runtime = createRuntimeDeps({
      findCommandDispatch: () => {
        dispatchCalls += 1;
    ...
    const exitCode = await runCli(["human", "--profile", "op"], runtime.deps);

    expect(exitCode).toBe(1);
    expect(dispatchCalls).toBe(0);
    expect(runtime.stdout).toEqual([]);
    expect(runtime.stderr).toEqual(["Missing value for --profile.\n"]);
  });
