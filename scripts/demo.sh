...
export OPCHAIN_HELPER_PATH="$root_dir/test/fixtures/bin/opchain-helper"
export OPCHAIN_OP_PATH="$root_dir/test/fixtures/bin/op"

printf 'Fixture-backed opchain demo\n'
printf 'This uses fake helper/op binaries from test fixtures, not your real 1Password account.\n'
printf 'HOME_DIR=%s\n' "$home_dir"
...
run_demo_command \
  "human secrets check" \
  env OPCHAIN_TEST_HELPER_TOKEN='token-for-human-default' \
...
run_demo_command \
  ...
  env OPCHAIN_TEST_HELPER_TOKEN='token-for-kevin-write' \
  bun run --silent "$root_dir/src/index.ts" kevin --write op item edit Stripe --vault Services
run_demo_command \
  "human expires add" \
  env OPCHAIN_TEST_HELPER_TOKEN='token-for-human-default' \
