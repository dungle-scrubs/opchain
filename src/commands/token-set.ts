...
import { setTokenWithSecurity } from "../token/security-provider.ts";
 ...
 */
export function parseTokenSetOptions(
...
export async function runTokenSet(
  ...
  const setResult = await setTokenWithSecurity({
  ...
  writeTelemetry(
    options,
    createTelemetryEvent("token.set", {
      identity: tokenSetOptions.identity,
      input_mode: tokenSetOptions.stdin ? "stdin" : "tty",
      profile: tokenSetOptions.profile,
    }),
  );
