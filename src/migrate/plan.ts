...
import { expandHomePath } from "../config/path-normalization.ts";
...
export type MigrationPlan = {
  ...
  readonly projectsDirHomeExpanded: boolean;
...
function buildMigratedConfigToml(
  ...
  const projectsDir = expandHomePath(legacyConfig.projectsDir ?? "~/dev");
...
export async function buildMigrationPlan(
  ...
  const outputLines = [
    ...
    `Map ${legacyConfig.readAccount ?? "(missing)"} -> primary.read`,
    `Map ${legacyConfig.writeAccount ?? "(missing)"} -> primary.write`,
  ...
  return {
    ...
    value: {
      ...
      projectsDirHomeExpanded:
        legacyConfig.projectsDir === null ||
        legacyConfig.projectsDir === "~" ||
        legacyConfig.projectsDir.startsWith("~/"),
